...
defaultSpeed:float = 10 #TODO: Change this/measure this
...
def turnFinch(turn:Turns):
    TURN_WAIT_TIME:float = 0.1 #TODO: check if this is enough?
...
def turnAndDetectWalls() -> Directions:
    ...
    if(wallPresentDirections[0] == False and wallPresentDirections[1] == False): #no wall on both sides!
        if(wallPresentDirections[0]>wallPresentDirections[1]):
...
def traversePath(directions:Directions, firstTime:bool = False):
    PERIODIC_WALL_CHECK_TIME:float = 1 #TODO: check this time

    global leftOrRightTurn
    leftOrRightTurn:int = 0 #0=left, 1=right

    if(firstTime):
        finch.setMotors(defaultSpeed, defaultSpeed)
    
    if(directions[0] == False & directions[1] == False):
    ...
    elif(directions[0] & directions[1]==False):
        turnFinch(Turns.RIGHT)
        leftOrRightTurn = 1
        print('Turning right as there is a wall to the left')
    elif(directions[1] & directions[0]==False):
        turnFinch(Turns.LEFT)
        leftOrRightTurn = 0
